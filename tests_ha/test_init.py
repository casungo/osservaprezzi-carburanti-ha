        ...
        "fuels": [
            {
                ...
                "insertDate": "2026-06-06T18:15:30Z",
...
async def test_config_entry_lifecycle_and_services(
    hass: HomeAssistant,
    monkeypatch,
) -> None:
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="UNION - BORGHESANO LUCCHESE",
    ...
    open_entity_id = registry.async_get_entity_id(
        "binary_sensor", DOMAIN, f"{STATION_ID}_open_closed"
    )
    service_entity_id = registry.async_get_entity_id(
        "binary_sensor", DOMAIN, f"{STATION_ID}_service_1"
    )

    assert price_entity_id is not None
    assert open_entity_id is not None
    assert service_entity_id is not None
    assert hass.states.get(price_entity_id).state == "1.798"
    assert hass.states.get(open_entity_id).state == "on"
    assert hass.states.get(service_entity_id).state == "on"
    ...
    assert all(hass.states.get(entity_id) is None for entity_id in entity_ids_after_reload)
